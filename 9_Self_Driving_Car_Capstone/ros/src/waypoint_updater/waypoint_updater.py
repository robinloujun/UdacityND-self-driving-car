...
LOOKAHEAD_WPS = 50  # Number of waypoints that will be published.
MAX_DECEL = 0.5
...
class WaypointUpdater(object):
    def __init__(self):
        ...
        rospy.loginfo('[WaypointUpdater] waypoint_updater loaded')
    ...
    def get_closest_waypoints_idx(self):
        ...
        closet_vec = np.array(closest_coord)
        prev_vec = np.array(prev_coord)
        pose_vec = np.array([x, y])

        val = np.dot(closet_vec-prev_vec, pose_vec-closet_vec)
    ...
    def decelerate_waypoints(self, waypoints, closest_idx):
        rospy.loginfo('[WaypointUpdater] decelerating ...')
        decl_waypoints = []
        ...
        for i, wp in enumerate(waypoints):
            ...
            decl_waypoints.append(p)

        return decl_waypoints

    def pose_cb(self, msg):
        # callback for pose info
        self.pose = msg

    def waypoints_cb(self, waypoints):
        # callback for base waypoints
        self.base_lane = waypoints
        if not self.waypoints_2d:  # initialize
            self.waypoints_2d = [[pt.pose.pose.position.x, pt.pose.pose.position.y]
                                 for pt in waypoints.waypoints]
